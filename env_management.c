...
/**
 * env_get_key - gets the value of an enviroment variable
...
char *env_get_key(char *key, data_of_program *data)
{
	...
	/* obtains the length of variable requested */
 ...
 * or create it if does not exit
 * @key: name of the variable to set
 * @value: new value
 * @data: struct of the program's dta
 * Return: 1 if the parameters are NULL, 2 if there is an error or 0 is success
 */
...
int env_set_key(char *key, char *value, data_of_program *data)
{
	...
	for (i = 0; data->env[i]; i++)
	{/* Iterates tnrough the environ and check for coincidence of the vame */
		if (str_compare(key, data->env[i], key_length) &&
				data->env[i][key_length] == '=')
	...
	if (is_new_key)
	{/* if the variable is new, it is create at end of actal list and we need*/
...
int env_remove_key(char *key, data_of_program *data)
{
	...
	{/* iterates through the environ and checks for coincidences */
		...
		{/* if key already exists, remove them */
			...
			/* put the NULL value at the new end of list the list */
			data->env[i - 1] = NULL;
			return (1);
		}
	}
